...
String serchField = request.getParameter("searchField");
...
if(serchWord != null){
	...
	param.put("searchField" , serchField);
	//클라이언트가 입력한 검색어
	param.put("searchWord" , serchWord);
}

//board테이블에 저장된 게시물의 갯수를 카운트한다.
...
//전체 페이지수를 계산한다. (전체게시물의갯수/ 페이지당게시물갯수 = 결과의 올림처리)
int totalPage = (int)Math.ceil((double)totalCount / pageSize);

/*
목록에 첫 진입시에는 페이지 관련 파라미터가 없는 상태이므로 무조건 1page로 지정한다.
만약 파라미터 pageNum이 있다면 request내장객체를 통해 받은 후 페이지번호로 저장한다.
...
//계산된 값을 DAO로 전달하기 위해 Map 컬렉션에 저장한다.
